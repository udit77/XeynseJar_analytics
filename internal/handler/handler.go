...
func New(router *httprouter.Router, jarUseCase jar.Resource) {
	...
	router.POST("/xeynseJar/analytics/consumption/calories", httpHandler(handler.getCaloriesConsumption))
	router.POST("/xeynseJar/analytics/consumption/macronutrients", httpHandler(handler.getMacroNutrientConsumption))
}
...
func (h handler) getCaloriesConsumption(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	w.Header().Set("Content-Type", "application/json")
	requestBody, err := ioutil.ReadAll(r.Body)
	if err != nil {
...
func (h handler) getMacroNutrientConsumption(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	w.Header().Set("Content-Type", "application/json")
	requestBody, err := ioutil.ReadAll(r.Body)
	if err != nil {
		utilErr := errUtil.BadError("", "error reading create mode request body: "+err.Error())
	...
	jarStatusResponse, err := h.jarUseCase.GetMacroNutrientConsumption(statusRequest)
	if err != nil {
		utilErr := errUtil.InternalServerError("", "error occurred in fetching home analytics: "+err.Error())
		response := entity.Response{Status: utilErr.HttpCode, Success: false, Message: "Fail", Error: utilErr}
		responseBytes, _ := json.Marshal(response)
